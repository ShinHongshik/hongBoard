...
static void bootInfo(cmd_can_t *p_cmd)
{
  boot_info.mode = 1;

  cmdCanSendResp(p_cmd, p_cmd->packet.cmd, CMD_OK, (uint8_t *)&boot_info, sizeof(boot_info_t));
}


static void bootVersion(cmd_can_t *p_cmd)
{
  ...
  cmdCanSendResp(p_cmd, p_cmd->packet.cmd, CMD_OK, (uint8_t *)&boot_version, sizeof(boot_version_t));
}


static void bootFirmVersion(cmd_can_t *p_cmd)
{
  cmdCanSendResp(p_cmd, p_cmd->packet.cmd, CMD_OK, (uint8_t *)(FLASH_ADDR_FIRM + FLASH_SIZE_VER), sizeof(firm_ver_t));
}

static void bootFirmErase(cmd_can_t *p_cmd)
{
  uint32_t addr = 0;
  uint32_t length = 0;
  cmd_can_packet_t *p_packet = &p_cmd->packet;
...
static void bootFirmWrite(cmd_can_t *p_cmd)
{
  uint32_t addr = 0;
  uint32_t length = 0;
  cmd_can_packet_t *p_packet = &p_cmd->packet;
...
static void bootFirmRead(cmd_can_t *p_cmd)
{
  uint32_t addr = 0;
  uint32_t length = 0;
  cmd_can_packet_t *p_packet = &p_cmd->packet;
  ...
  cmdCanSendResp(p_cmd, p_cmd->packet.cmd, err_code, p_packet->data, length);
}


static void bootFirmVerify(cmd_can_t *p_cmd)
{
  uint32_t addr = 0;
  uint32_t length = 0;
  uint16_t crc;
...
static void bootFirmUpdate(cmd_can_t *p_cmd)
{
  cmdCanSendResp(p_cmd, p_cmd->packet.cmd, CMD_OK, NULL, 0); 
  delay(100);
...
static void bootFirmBegin(cmd_can_t *p_cmd)
{
  cmd_can_packet_t *p_packet = &p_cmd->packet;
  uint16_t err_code = CMD_OK;
...
static void bootFirmEnd(cmd_can_t *p_cmd)
{
  uint16_t err_code = CMD_OK;
...
void cmdBootUpdate(cmd_can_t *p_cmd)
{
  enum 
  {
    STATE_IDLE,
  ...
  if (state == STATE_BEGIN)
  {
    ...
    //   lcdDrawFillRect(0, 32, LCD_WIDTH, 32, white);     
    //   lcdPrintfRect  (0, 32, LCD_WIDTH, 32, black, 32, LCD_ALIGN_H_CENTER|LCD_ALIGN_V_CENTER, 
    //     "F/W UPDATE");
      cliPrintf("F/W UPDATE\r\n");

      int percent;
      percent = fw_receive_size * 100 / boot_begin.fw_size;

      // lcdPrintfResize(0, 16*6 , white, 32, "%d KB", boot_begin.fw_size/1024);
      // if (p_cmd->packet.cmd == BOOT_CMD_FW_BEGIN)
        // lcdPrintfResize(0, 16*8 , red, 32, "ERASE...");  
      // else
      //   lcdPrintfResize(0, 16*8 , white, 32, "%d %%", fw_receive_size * 100 / boot_begin.fw_size);
      // lcdDrawRect    (0, 16*11, LCD_WIDTH, 32, white);
      // lcdDrawFillRect(0, 16*11, LCD_WIDTH * percent / 100, 32, green);     
...
bool cmdBootProcess(cmd_can_t *p_cmd)
{
  bool ret = true;
