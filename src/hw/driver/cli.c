...
void clitest(cli_args_t *args)
{
  ...
  if (args->argc == 1)
  {
    // volume = (uint16_t) args->getData(1);
    uartPrintf(_DEF_UART2, "%s\n",args->getStr(0));
      //cliprintf("ok\r\n");
    #if 0 
    if(args->isStr(0, "volume") == true)
    {
      volume_test(volume);
      //cliprintf("ok\r\n");
    }
    else if (args->isStr(0, "mute") == true)
    {
      up_down_test(MUTE);
      //cliprintf("ok\r\n");
    }
    else if (args->isStr(0, "up") == true)
    {
      up_down_test(UP);
      //cliprintf("ok\r\n");
    }
    else if (args->isStr(0, "down") == true)
    {
      up_down_test(DOWN);
      //cliprintf("ok\r\n");
    }
